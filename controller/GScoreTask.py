...
class ScoreTask():
    ...
    def __init__(self, methods):
        try:
            ...
            order = request.args.get('order', 's_task_no')
            ...
            print("error: %s" % (e))
            self.app = redirect("/ScoreTask?page=1")
    ...
    def Show(self, methods, aFunc_args):
        ...
        try:
            ...
            if (aFunc_args['search_options'] == 'All') or (aFunc_args['search_key'] == ''):
                aScoreTasks = self.oDB.select_all("score_task", '1=1', 'ORDER BY %s %s LIMIT %s, %s' % (aFunc_args['order'], aFunc_args['sort'], str(pagestart), str(aFunc_args['showcolnum'])))
                aScoreTasks_count = self.oDB.count_datas("score_task")

            elif (aFunc_args['search_options'] != 'All'):
                aScoreTasks = self.oDB.select_all("score_task", "%s LIKE '%s'" % (aFunc_args['search_options'], "%" + str(aFunc_args['search_key']) + "%"), "ORDER BY %s %s LIMIT %s, %s" % (aFunc_args['order'], aFunc_args['sort'], str(pagestart), str(aFunc_args['showcolnum'])))

                aScoreTasks_count = self.oDB.count_datas("score_task", "%s = '%s'" % (aFunc_args['search_options'], str(aFunc_args['search_key'])))
            else:
                aScoreTasks = self.oDB.select_all("score_task", f"{aFunc_args['search_options']} LIKE '%{aFunc_args['search_key']}%' ORDER BY {aFunc_args['order']} {aFunc_args['sort']} LIMIT {str(pagestart)}, {str(aFunc_args['showcolnum'])}")

                aScoreTasks_count = self.oDB.count_datas("score_task", f"{aFunc_args['search_options']} LIKE '%{str(aFunc_args['search_key'])}%'")

            iCount_datas = int(aScoreTasks_count['total'])
            ...
            if methods == 'GET' and aFunc_args['func'] == '':
                self.app = render_template('score_task_list.html',
                            ...
                            aScoreTasks=aScoreTasks,
                            colstart=(int(pagestart) + 1),
                            sfunc='ScoreTask')

            elif methods == 'GET' and aFunc_args['func'] == 'dosort':
                self.app = redirect("/ScoreTask?page=%s&items=%s&order=%s&sort=%s&search=%s&search_key=%s" % (str(iPage), str(
                    aFunc_args['showcolnum']), aFunc_args['order'], aFunc_args['sort'], aFunc_args['search_options'], aFunc_args['search_key']), code=302)

            elif methods == 'POST':
                self.app = redirect("/ScoreTask?page=1&items=%s&order=s_task_no&sort=ASC&search=%s&search_key=%s" % (
